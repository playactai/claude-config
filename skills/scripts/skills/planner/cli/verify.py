...
layer for the LLM-asserted verdict: it requires ALL THREE checks in one call, a
typed pass|fail, and a non-empty summary per check; runs a light status/summary
consistency check; and bumps the failed-cycle counter that lets the gate escalate
instead of looping a red suite forever. It cannot make an LLM honest, but it makes
the honest record the only well-formed one.
"""
...
# A nonzero "N failed" / "N error(s)" in a summary contradicts a pass; a clean
# run ("all checks passed", "0 errors", only "N passed") contradicts a fail.
...
_ONLY_PASSED = re.compile(r"\b\d+\s+passed\b")
...
def _summary_contradiction(status: str, summary: str) -> str | None:
    """Return a reason string when status and summary clearly disagree, else None."""
    s = summary.lower()
    if status == "pass":
        if _NONZERO_FAIL.search(s):
            return "summary reports failures/errors but status is 'pass'"
        return None
    # status == "fail"
    if "all checks passed" in s or "0 errors" in s:
        return "summary reports a clean run but status is 'fail'"
    if _ONLY_PASSED.search(s) and not _NONZERO_FAIL.search(s):
        return "summary reports only passes but status is 'fail'"
    return None
...
def record(state_dir: str, statuses: dict[str, str], summaries: dict[str, str]) -> None:
    ...
    the prior count (so the gate escalates at QR_ITERATION_LIMIT); an all-pass
    record leaves it at >=1 (value then irrelevant -- the gate passes).
    """
    for check in VERIFY_CHECKS:
        summary = summaries[check].strip()
        if not summary:
            _error_exit(f"--{check}-summary must be non-empty (paste the command's summary line)")
    ...
    results = [
        VerifyResult.model_validate(
            {"check": check, "status": statuses[check], "summary": summaries[check].strip()}
        )
        for check in VERIFY_CHECKS
    ]
    ...
    # a passing record never inflates it.
    prior = load_verify_state(state_dir)
    prior_iter = prior.iteration if prior else 0
    new_iter = prior_iter + 1 if any_fail else prior_iter

    vf = VerifyFile(iteration=new_iter, results=results)
