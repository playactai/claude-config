...
| ------------- | ----------------------------------------------------------------------------- | ------------------------------------------------------------------ |
| `phases.py`   | `QR_PHASES` registry + `get_phase_config()` — single source of truth for each phase's decompose/verify script paths and workflow | Adding a phase, changing script paths                              |
...
| `utils.py`    | `load_qr_state`, `query_items`, `by_status`, `by_blocking_severity`, `increment_qr_iteration`, `has_qr_failures_from_state`, `qr_file_exists`, formatting helpers | Reading/writing qr-{phase}.json, filtering items by status/severity |
