...
class TestFixModeIterationBanner:
    ...
    def test_step_2_banner_reads_iteration_3(self, tmp_path: Path):
        _write_qr_state(tmp_path, "impl-code", iteration=3, items=[
            {"id": "impl-001", "check": "x", "status": "FAIL", "severity": "MUST",
             "scope": "*", "finding": "broken"},
        ])
        out = format_output(step=2, state_dir=str(tmp_path),
                            qr_status=None, reconciliation_check=False)
    ...
    def test_step_6_banner_reads_iteration_4(self, tmp_path: Path):
        _write_qr_state(tmp_path, "impl-docs", iteration=4, items=[
            {"id": "docs-001", "check": "x", "status": "FAIL", "severity": "MUST",
             "scope": "*", "finding": "broken"},
        ])
        out = format_output(step=6, state_dir=str(tmp_path),
                            qr_status=None, reconciliation_check=False)
    ...
    def test_no_fix_mode_when_all_pass(self, tmp_path: Path):
        _write_qr_state(tmp_path, "impl-code", iteration=1, items=[
            {"id": "impl-001", "check": "x", "status": "PASS", "severity": "MUST",
             "scope": "*", "finding": None},
        ])
        out = format_output(step=2, state_dir=str(tmp_path),
                            qr_status=None, reconciliation_check=False)
...
class TestRemovedCliFlags:
    ...
    def test_rejected(self, flag: str, value: str | None,
...
class TestEmptyQrVerifyRouting:
    ...
    def test_empty_items_uses_single_next_step(self, tmp_path: Path):
        ...
        _write_qr_state(tmp_path, "impl-code", iteration=5, items=[
            {"id": "impl-001", "check": "trivial nit", "status": "FAIL",
             "severity": "COULD", "scope": "*", "finding": "cosmetic"},
        ])
        qr = QRState(iteration=5, state=LoopState.INITIAL)
        out = format_qr_verify(step=4, phase="impl-code",
                                state_dir=str(tmp_path), qr=qr)
    ...
    def test_non_empty_items_keeps_branching(self, tmp_path: Path):
        ...
        _write_qr_state(tmp_path, "impl-code", iteration=1, items=[
            {"id": "impl-001", "check": "real check", "status": "TODO",
             "severity": "MUST", "scope": "*"},
        ])
